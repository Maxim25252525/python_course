...
from final_project.my_classes import Computer, Shot, User
from final_project.my_functions import (check_destroyed_tank, check_exit,
                                        check_hit, check_input, conv_cmd,
                                        converted_coords, coordinates_dict,
                                        create_tanks, print_fields, tip)
            ...
            case "старт":
                ...
                while computer_field.tanks and user_field.tanks:
                    ...
                    if turn == "user":
                        ...
                        command = conv_cmd(
                            input(
                                "Введите координаты вашего выстрела "
                        ...
                        is_used = False
                        while not check_input(command, "shot",
                                              computer_field):
                            sleep(0.5)
                        ...
                        if check_hit(user_shot, computer_field, "tank")[0]:
                            user_shot.hit = True
                            computer_field.shots.append(user_shot)
                            print_fields(user_field, computer_field)
                            # Проверяем, уничтожен ли танк.
                            if check_destroyed_tank(computer_field, user_shot)[
                                0
                            ]:
                                tank_id = 0
                                ...
                                d_tank = computer_field.tanks[
                                    tank_id
                                ]  # Уничтоженный танк.
                                computer_field.tanks.pop(
                                    tank_id
                                )  # Удаляем танк.
                                r1 = d_tank.rows[0] + 1
                                r2 = d_tank.rows[1] + 1
                    ...
                    # Алгоритм для компьютера
                        ...
                        while check_hit(computer_shot, user_field, "tank")[0]:
                            # Танк уничтожен.
                            if check_destroyed_tank(user_field, computer_shot)[
                                0
                            ]:
                                tank_id = 0
                ...
                sleep(0.5)
                command = conv_cmd(
                    input("Хотите сыграть еще раз? [да/нет]\n> ")
                )
                if command == "да":
                    command = "старт"
                else:
