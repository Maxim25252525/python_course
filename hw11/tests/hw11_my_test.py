...
@pytest.mark.parametrize(
    ("name", "carat", "result"),
    [("Сапфир", 2.5, ["Название: Сапфир", "Карат: 2.5"])],
)
def test_gemstone(name, carat, result):
    my_brilliant = task1.Gemstone(name, carat)
    assert (my_brilliant.get_name(), my_brilliant.get_carat()) == (*result,)


@pytest.mark.parametrize(
    ("name", "carat", "diameter", "result"),
    [
        (
            "Сапфир",
            2.5,
            3.1,
            ["Название: Сапфир", "Карат: 2.5", "Диаметр: 3.1 мм"],
        )
    ],
)
...
@pytest.mark.parametrize(
    ("name", "carat", "diameter", "cost", "result"),
    [
        (
            "Сапфир",
            2.5,
