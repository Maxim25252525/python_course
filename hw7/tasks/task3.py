...
def create_weather_report(degrees: list[float]) -> dict[str, ...]:
    """Создает отчет о температуре.

    Args:
        degrees: Список с температурой.

    Returns:
        Словарь с уникальными значениями температуры и средней температурой.
    """
    return {
        "unique_degrees": set(degrees),
        "average_temperature": sum(degrees) / len(degrees),
    }
...
if __name__ == "__main__":
    ...
    print("Отчет о температуре:", end=" ")
    print(*report["unique_degrees"])
    print(f"Среднее значение температуры: {report['average_temperature']:.2f}")
